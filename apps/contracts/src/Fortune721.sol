    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory svg = string(
            abi.encodePacked(
                ...
                '<text x="500" y="350" text-anchor="middle" fill="#fff" font-size="32" font-family="',
                bytes(fontURI).length > 0 ? "Custom" : "serif",
                '">',
                text1,
                '</text>',
                '<text x="500" y="500" text-anchor="middle" fill="#fff" font-size="32" font-family="',
                bytes(fontURI).length > 0 ? "Custom" : "serif",
                '">',
                text2,
                '</text>',
                '<text x="500" y="650" text-anchor="middle" fill="#fff" font-size="32" font-family="',
                bytes(fontURI).length > 0 ? "Custom" : "serif",
                '">',
                text3,
                '</text>',
