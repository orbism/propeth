...
contract DeployComplete is Script {
    function run() external {
        ...
        console.log("MockRandomness deployed: %s", address(randomness));

        // 2. Deploy Pack1155 with card metadata URI
        // TODO: Replace with your actual IPFS CID for card metadata
        // Format: ipfs://CID/{id}.json where {id} is 0-14
        Pack1155 pack = new Pack1155(
            "ipfs://bafybeif7pvyusbmmzoqhnsoafbh4zmys2myskgche7zxnia2svwp7ap5w4/",  // UPDATE THIS!
            15,                               // maxCardId (0-14)
            30000000000000000,                // 0.03 ETH
            deployer,
            address(randomness)
        );
        console.log("Pack1155 deployed: %s", address(pack));

        // 3. Deploy Fortune721 with background image
        Fortune721 fortune = new Fortune721("bafybeih3foatdgwucci5whnyhewkpc7c7w4y37birvsla6gjrdmg7d62va");
        console.log("Fortune721 deployed: %s", address(fortune));
        
        // Font is handled by contract - uses monospace system font by default
        console.log("Fortune font: monospace (system default)");

        // 4. Deploy BurnRedeemGateway
        BurnRedeemGateway gateway = new BurnRedeemGateway(address(pack));
        console.log("BurnRedeemGateway deployed: %s", address(gateway));
